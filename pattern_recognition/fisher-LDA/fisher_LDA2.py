...
def fisher_calculate(i):
    
    t1 = np.matrix(train_set[i])
    if i == 0:
        t2 = np.matrix(train_set[1] + train_set[2])
    elif i == 1:
        t2 = np.matrix(train_set[0] + train_set[2])
    elif i == 2:
        t2 = np.matrix(train_set[0] + train_set[1])
    ...
    for pattern in test_set:
        ...
        w = i if y >= yt else -1
        index = 0 if w == i else 1
        recognition_numi[index] += 1
        if w == pattern[4] or (w == -1 and pattern[4] !=  i):
            correct_num[index] += 1
...
value = fisher_calculate(0)
value += fisher_calculate(1)
value += fisher_calculate(2)
